...
namespace Contracts.Api.Controllers
{
    ...
    public class AuxiliarController : ControllerBase
    {
        ...
        [HttpGet("aux/clubes/real/new")]
        public IActionResult GetRealClubes([FromQuery] FilterDto filterDto)
        {
            var jsonSerializerSettings = new JsonSerializerSettings
            {
                MissingMemberHandling = MissingMemberHandling.Ignore
            };
            var clubes = JsonConvert.DeserializeObject<List<ClubeDto>>(clubesJson, jsonSerializerSettings);

            clubes?.Shuffle();
            clubes = clubes?.Take(filterDto.Quantidade).ToList();

            return Ok(clubes);
        }

        [HttpGet("aux/clubes/fake/new")]
        public async Task<IActionResult> GetFakeClubes([FromQuery] FilterDto filterDto)
        {
            var clubes = await _clubeService.GetNewFakes(filterDto.Quantidade);
            return Ok(clubes);
        }
        ...
        [HttpGet("aux/clubes/mix")]
        public async Task<IActionResult> GetMixClubes([FromQuery] FilterDto filterDto, [FromQuery] bool withChanges)
        {
            return Ok(await _clubeService.GetMix(filterDto.Quantidade, withChanges));
        }
